...
public static class AdminSeeder
{
    public static async Task SeedAdminAsync(IntelliCampusDbContext context, IPasswordService passwordService)
    {
        ...
        // Assign office hours rooms to instructors
        var allRooms = await context.Rooms.ToListAsync();
        instructors = await context.Instructors.ToListAsync();
        instructors[0].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Hall A1")?.RoomId;
        instructors[1].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Hall B2")?.RoomId;
        instructors[2].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Lab 101")?.RoomId;
        instructors[3].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Lab 102")?.RoomId;
        instructors[4].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Hall D1")?.RoomId;
        instructors[5].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Lab 301")?.RoomId;
        instructors[6].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Hall E1")?.RoomId;
        instructors[7].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Lab 401")?.RoomId;
        instructors[8].OfficeHoursRoomId = allRooms.FirstOrDefault(r => r.RoomName == "Hall F1")?.RoomId;
        await context.SaveChangesAsync();
        ...
        // Exams
        // ??????? Exams ???????
        await UpsertExamsAsync(context);
    }
    ...
    private static async Task UpsertRoomsAsync(IntelliCampusDbContext context)
    {
        ...
        {
            new() { RoomName = "Hall A1", RoomNameAr = "┘é╪º╪╣╪⌐ A1", Capacity = 120 },
            new() { RoomName = "Hall A2", RoomNameAr = "┘é╪º╪╣╪⌐ A2", Capacity = 100 },
            new() { RoomName = "Hall B2", RoomNameAr = "┘é╪º╪╣╪⌐ B2", Capacity = 90 },
            new() { RoomName = "Hall C1", RoomNameAr = "┘é╪º╪╣╪⌐ C1", Capacity = 150 },
            new() { RoomName = "Hall D1", RoomNameAr = "┘é╪º╪╣╪⌐ D1", Capacity = 80 },
            new() { RoomName = "Hall D2", RoomNameAr = "┘é╪º╪╣╪⌐ D2", Capacity = 70 },
            new() { RoomName = "Hall E1", RoomNameAr = "┘é╪º╪╣╪⌐ E1", Capacity = 100 },
            new() { RoomName = "Hall F1", RoomNameAr = "┘é╪º╪╣╪⌐ F1", Capacity = 120 },
            new() { RoomName = "Lab 101", RoomNameAr = "┘à╪╣┘à┘ä 101", Capacity = 30 },
            new() { RoomName = "Lab 102", RoomNameAr = "┘à╪╣┘à┘ä 102", Capacity = 30 },
            new() { RoomName = "Lab 103", RoomNameAr = "┘à╪╣┘à┘ä 103", Capacity = 25 },
            new() { RoomName = "Lab 104", RoomNameAr = "┘à╪╣┘à┘ä 104", Capacity = 25 },
            new() { RoomName = "Lab 105", RoomNameAr = "┘à╪╣┘à┘ä 105", Capacity = 30 },
            new() { RoomName = "Lab 201", RoomNameAr = "┘à╪╣┘à┘ä 201", Capacity = 35 },
            new() { RoomName = "Lab 301", RoomNameAr = "┘à╪╣┘à┘ä 301", Capacity = 30 },
            new() { RoomName = "Lab 302", RoomNameAr = "┘à╪╣┘à┘ä 302", Capacity = 25 },
            new() { RoomName = "Lab 303", RoomNameAr = "┘à╪╣┘à┘ä 303", Capacity = 30 },
            new() { RoomName = "Lab 401", RoomNameAr = "┘à╪╣┘à┘ä 401", Capacity = 35 },
            new() { RoomName = "Lab 402", RoomNameAr = "┘à╪╣┘à┘ä 402", Capacity = 30 },
        };
    ...
    private static async Task UpsertExamsAsync(IntelliCampusDbContext context)
    {
        ...
        if (courses.Count == 0 || rooms.Count == 0)
            return;
        ...
        var cs101 = courses.FirstOrDefault(c => c.CourseCode == "CS-101");

        var hallA1 = rooms.FirstOrDefault(r => r.RoomName == "Hall A1");
        var hallB2 = rooms.FirstOrDefault(r => r.RoomName == "Hall B2");
        var hallC1 = rooms.FirstOrDefault(r => r.RoomName == "Hall C1");
        var hallD1 = rooms.FirstOrDefault(r => r.RoomName == "Hall D1");
        var hallA2 = rooms.FirstOrDefault(r => r.RoomName == "Hall A2");

        if (cs201 is null || cs301 is null || cs302 is null || is202 is null)
            return;

        if (hallA1 is null || hallB2 is null || hallC1 is null || hallD1 is null)
            return;
        ...
        var exams = new List<Exam>
        {
            ...
            new() { Title = "Data Structures Midterm", Description = "Covers arrays, linked lists, stacks, queues, and recursion", ExamType = ExamType.Midterm, Status = ExamStatus.Upcoming, Date = now.AddDays(30), Time = new TimeSpan(9, 0, 0), DurationMinutes = 120, MaxGrade = 100, TotalMarks = 50, RoomId = hallA1.RoomId, CourseId = cs201.CourseId, CreatedAt = now },
            new() { Title = "Data Structures Final", Description = "Comprehensive exam covering all topics", ExamType = ExamType.Final, Status = ExamStatus.Upcoming, Date = now.AddDays(60), Time = new TimeSpan(13, 0, 0), DurationMinutes = 150, MaxGrade = 100, TotalMarks = 60, RoomId = hallA1.RoomId, CourseId = cs201.CourseId, CreatedAt = now },

            // CS-301 DBMS
            new() { Title = "Database Systems Midterm", Description = "ER diagrams, relational model, SQL", ExamType = ExamType.Midterm, Status = ExamStatus.Upcoming, Date = now.AddDays(32), Time = new TimeSpan(10, 0, 0), DurationMinutes = 90, MaxGrade = 100, TotalMarks = 40, RoomId = hallB2.RoomId, CourseId = cs301.CourseId, CreatedAt = now },
            new() { Title = "Database Systems Final", Description = "Normalization, transactions, indexing", ExamType = ExamType.Final, Status = ExamStatus.Upcoming, Date = now.AddDays(65), Time = new TimeSpan(11, 0, 0), DurationMinutes = 120, MaxGrade = 100, TotalMarks = 50, RoomId = hallB2.RoomId, CourseId = cs301.CourseId, CreatedAt = now },

            // CS-302 Computer Networks
            new() { Title = "Computer Networks Midterm", Description = "OSI model, TCP/IP, IP addressing", ExamType = ExamType.Midterm, Status = ExamStatus.Upcoming, Date = now.AddDays(35), Time = new TimeSpan(11, 0, 0), DurationMinutes = 90, MaxGrade = 100, TotalMarks = 40, RoomId = hallC1.RoomId, CourseId = cs302.CourseId, CreatedAt = now },
            new() { Title = "Computer Networks Final", Description = "Routing protocols, network security, application layer", ExamType = ExamType.Final, Status = ExamStatus.Upcoming, Date = now.AddDays(70), Time = new TimeSpan(10, 0, 0), DurationMinutes = 120, MaxGrade = 100, TotalMarks = 50, RoomId = hallC1.RoomId, CourseId = cs302.CourseId, CreatedAt = now },

            // IS-202 Web Development
            new() { Title = "Web Development Midterm", Description = "HTML5, CSS3, responsive design", ExamType = ExamType.Midterm, Status = ExamStatus.Upcoming, Date = now.AddDays(34), Time = new TimeSpan(9, 0, 0), DurationMinutes = 90, MaxGrade = 100, TotalMarks = 40, RoomId = hallD1.RoomId, CourseId = is202.CourseId, CreatedAt = now },
            new() { Title = "Web Development Final", Description = "JavaScript, DOM, React basics", ExamType = ExamType.Final, Status = ExamStatus.Upcoming, Date = now.AddDays(68), Time = new TimeSpan(13, 0, 0), DurationMinutes = 120, MaxGrade = 100, TotalMarks = 50, RoomId = hallD1.RoomId, CourseId = is202.CourseId, CreatedAt = now },

            // CS-101 Introduction to Programming (completed past exams)
            new() { Title = "Programming Fundamentals Midterm", Description = "Covers basics of C# programming", ExamType = ExamType.Midterm, Status = ExamStatus.Completed, Date = now.AddDays(-60), Time = new TimeSpan(9, 0, 0), DurationMinutes = 90, MaxGrade = 100, TotalMarks = 40, RoomId = hallA2.RoomId, CourseId = cs101.CourseId, CreatedAt = now },
            new() { Title = "Programming Fundamentals Final", Description = "Comprehensive programming exam", ExamType = ExamType.Final, Status = ExamStatus.Completed, Date = now.AddDays(-30), Time = new TimeSpan(10, 0, 0), DurationMinutes = 120, MaxGrade = 100, TotalMarks = 50, RoomId = hallA2.RoomId, CourseId = cs101.CourseId, CreatedAt = now },
        };
